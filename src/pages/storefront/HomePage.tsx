  ...
  return (
    <div className="bg-white pb-20 animate-in fade-in duration-500">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-3 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2.5 sm:gap-4">
           {categories.filter(cat => cat.status !== 'Inactive' && cat.status !== 'inactive').slice(0, 12).map((cat, idx) => (
             ...
             >
                ...
                <h3 className="font-bold text-slate-800 text-xs sm:text-sm line-clamp-1">{cat.name}</h3>
                <p className="text-[10px] sm:text-[11px] font-semibold text-slate-500 mt-0.5">{cat.count || 0} Products</p>
             </div>
           ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 border-t border-slate-100">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Top Featured Brands</h2>
            <p className="text-slate-500 text-xs sm:text-sm mt-1">Authentic products directly from official manufacturers</p>
          </div>
          <button onClick={() => onNavigate('Products')} className="text-blue-600 font-bold text-xs sm:text-sm hover:underline flex items-center gap-1">
            Browse All <ArrowRight className="w-4 h-4" />
          </button>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-3xl p-5 sm:p-8 text-white shadow-xl flex flex-col lg:flex-row gap-8 items-center">
          <div className="lg:w-1/3">
            ...
            <h2 className="text-2xl sm:text-3xl font-black mb-3 leading-tight">Limited Time Discount Offers</h2>
            <p className="text-blue-100 text-xs sm:text-sm mb-6">Massive discounts on daily groceries and essentials in Sultanpur. Offer ends soon!</p>
            
            {/* Countdown Box */}
            <div className="grid grid-cols-4 gap-2 max-w-xs mb-4">
              <div className="text-center w-full">
                <div className="bg-white text-blue-700 font-black text-xl sm:text-2xl px-2 py-1.5 rounded-xl shadow-inner mb-1">{timeLeft.days}</div>
                <div className="text-[9px] sm:text-[10px] tracking-widest font-bold uppercase text-blue-100">Days</div>
              </div>
              <div className="text-center w-full">
                <div className="bg-white text-blue-700 font-black text-xl sm:text-2xl px-2 py-1.5 rounded-xl shadow-inner mb-1">{timeLeft.hours}</div>
                <div className="text-[9px] sm:text-[10px] tracking-widest font-bold uppercase text-blue-100">Hours</div>
              </div>
              <div className="text-center w-full">
                <div className="bg-white text-blue-700 font-black text-xl sm:text-2xl px-2 py-1.5 rounded-xl shadow-inner mb-1">{timeLeft.minutes}</div>
                <div className="text-[9px] sm:text-[10px] tracking-widest font-bold uppercase text-blue-100">Minutes</div>
              </div>
              <div className="text-center w-full">
                <div className="bg-white text-blue-700 font-black text-xl sm:text-2xl px-2 py-1.5 rounded-xl shadow-inner mb-1">{timeLeft.seconds}</div>
                <div className="text-[9px] sm:text-[10px] tracking-widest font-bold uppercase text-blue-100">Seconds</div>
              </div>
            </div>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
         <div className="flex items-end justify-between gap-4 mb-6 sm:mb-8">
            <div>
               <h2 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Popular Products</h2>
               <p className="text-slate-500 mt-0.5 sm:mt-1 text-xs sm:text-sm">Most loved by Sultanpur customers.</p>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
         <div className="bg-slate-900 rounded-3xl p-6 md:p-12 text-white flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8 overflow-hidden relative">
            ...
            <div className="relative z-10 max-w-xl text-center md:text-left">
               <span className="inline-block px-3 py-1 bg-amber-500 text-black font-bold text-xs uppercase tracking-wider rounded border border-amber-400 mb-3 sm:mb-4">Special Offer</span>
               <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4">Get up to 50% Off<br/>On Weekly Essentials</h2>
               <p className="text-slate-400 text-xs sm:text-sm mb-6 sm:mb-8 max-w-md mx-auto md:mx-0">Stock up your pantry with fresh produce and daily needs at unbeatable prices from top vendors in Sultanpur.</p>
               <button onClick={() => onNavigate('Products')} className="px-6 py-3 bg-white text-slate-900 font-bold rounded-xl text-xs sm:text-sm hover:bg-slate-100 transition-colors cursor-pointer shadow-md">
                  Claim Offer Now
               </button>
            </div>
            <div className="relative z-10 w-36 h-36 sm:w-48 sm:h-48 md:w-64 md:h-64 rounded-full overflow-hidden border-4 sm:border-8 border-white/10 shrink-0 bg-gradient-to-tr from-amber-500 to-rose-600 flex flex-col items-center justify-center text-white shadow-2xl">
               <Gift className="w-8 h-8 sm:w-12 sm:h-12 mb-1" />
               <span className="text-2xl sm:text-3xl font-black">50%</span>
               <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">OFF</span>
            </div>
         </div>
      </section>
